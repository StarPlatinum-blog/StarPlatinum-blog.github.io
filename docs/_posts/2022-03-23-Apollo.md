...
- 反射式融合：通过协程对Channel进行事件轮询，对接收到的感知数据进行融合；
...
3. 对取出的感知帧进行融合；
   1. 对前景目标进行融合`FuseForegroundTrack`；
      ...
      4. 使用未匹配的传感器目标创建新的轨迹
         ...
         3. 使用传感器帧的前景目标初始化一个概率跟踪器`PbfTracker`，并加入跟踪器列表；（这个跟踪器将后面轨迹被匹配到或没有被匹配到时在前面两步中用于更新轨迹）
...
[DS证据理论](https://en.wikipedia.org/wiki/Dempster%E2%80%93Shafer_theory)（DST: Dempster-Shafer theory）：Dempster-Shafer 理论是贝叶斯主观概率理论的推广。 信任函数将一个问题的信任度建立在相关问题的主观概率上。 置信度本身可能具有也可能不具有概率的数学特性； 它们有多少不同取决于这两个问题的相关程度。 换句话说，它是一种表示认知合理性的方法，但它可以产生与使用概率论得出的答案相矛盾的答案。

1. 假设空间/识别框架（fod: frame of discernment）：一个包含了若干个假设的集合，集合中的元素是完备的且互不相交；
...
3. 信任函数/信度函数（Belief function）

##### 存在性融合实现

> modules/perception/fusion/common/dst_evidence.h
...
- 存在性：`EXIST` `NEXIST` `EXISTUNKNOWN`

- 关注目标是否在摄像头判断区域（TOIC: target of interest in camera judgement）：`TOIC` `NTOIC` `TOICUNKNOWN`
...
- 轨迹与目标最大匹配距离（max\_match\_dist）：4.0

关联概率`association_prob`：

- 摄像头：1.0
- 其他传感器：$1-\frac{distance_{match}}{distance_{max\_match\_dist}} $

传感器可靠性`sensor_reliablity`：
...
- 毫米波雷达：由速度计算距离衰减`ComputeFeatureInfluence(measurement);`

  1. 计算速度因子：$factor_v = v > 4.0\ ?\ sigmoid(v) : 0$
  2. 由速度计算距离衰减：$decay = factor_v \times existance\_confidence_{obj}$
...
4. 利用DST计算证据合成：
   $$
   \hat{exist_{fused}} = \hat{exist_{fused}^{-}} + exist\_prob \times association\_prob
   $$

5. （如果感知目标来自摄像头）根据最新的激光雷达目标（没有则用最近的毫米波目标）在摄像头视野中的比例，利用DST更新TOIC；

6. 根据证据合成计算融合目标的存在概率和TOIC概率。（TODO：证据->概率的细节）
