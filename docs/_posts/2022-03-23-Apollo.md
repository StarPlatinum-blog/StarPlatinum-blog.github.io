...
关联概率`association_prob`：`match_distance`为用于融合的传感器帧与其匹配的场景中保存的轨迹之间的距离。
...
6. 根据证据合成计算融合目标的存在概率和TOIC概率。`UpdateExistenceState`
   ...
   1. 计算TOIC概率：
   2. 计算融合目标的存在概率：`GetExistenceProbability()`
...
| fod_loc_                  | 识别框架中最后一个元素的下标                                 | 0     | size\_t                  |
...
| combination_relations_    | 存储识别框架中元素之间存在交集关系且交集也是识别框架中元素的邻接表，行下标表示元素A，行中存在下标i，表示以i为下标的元素B和A存在交集，且其交集也是识别框架中的元素，即$B\cap A \neq \emptyset $且$B \cap A \subseteq \Theta$ |       | {size\_t, size\_t}[\][\] |
