...
`Intention` 不暴露内部结构的同时，可以让外部客户代码透明地访问其中包含的元素。

`Definition` 提供一种方法顺序访问一个聚合对象中的各个元素，而又不暴露该对象的内部表示。

`Hint` GoF中通过面向对象实现的迭代器已经过时了，STL里使用模版的方法实现的迭代器是现代的方法。

`Hint` 模版实现的是编译期多态，性能比虚函数的运行时多态更高。

`Hint` 在有了泛型编程的迭代器后C++ Programmer再也不用运行时多态的迭代器了。但是在其他语言中可能存在运行时多态的迭代器。

`Hint` iterator模式是C++ `STL`中迭代器设计的思想。可以通过学习`STL`源码来学习迭代器的原理。
...
### 模式 20：Chain of Responsibility

`Intention` 一个请求可能被多个对象处理，但是每个请求在运行时只能有一个接受者，如果显式指定，将必不可少地带来请求发送者与接受者的紧耦合。
...
1. 应用场合：一个请求可能有多个接收者，但是最后真正的接收者只有一个，这时候请求发送者与接收者的耦合有可能出现“变化脆弱”的症状，职责链的目的就是将二者解耦，从而更好地应对变化；
